    ...
    <div style={{ padding: isMobile ? '16px' : '24px' }}>
      ...
      {activeTab === 'visaSettings' && isAdmin && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '16px', padding: '20px', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                {appSettings.visaDurations.uae.map((d, idx) => {
                  ...
                  const currency = typeof d === 'object' ? d.currency : '$';
                  return (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(245,158,11,0.15)', padding: '12px', borderRadius: '8px', border: '1px solid rgba(245,158,11,0.3)' }}>
                      <span style={{ fontSize: '13px', color: '#f59e0b', fontWeight: '500' }}>{name}</span>
                      <span style={{ fontSize: '14px', color: '#f59e0b', fontWeight: '600' }}>{price} {currency}</span>
                    </div>
                  );
                })}
              </div>
          ...
          {/* Amerika */}
          {(appSettings?.visaDurations?.usa || []).length > 0 && (
            <div style={{ background: 'rgba(59,130,246,0.1)', padding: '14px', borderRadius: '10px', border: '1px solid rgba(59,130,246,0.2)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                <span style={{ fontSize: '18px' }}>🇺🇸</span>
                <span style={{ fontSize: '13px', fontWeight: '600', color: '#3b82f6' }}>Amerika</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {appSettings.visaDurations.usa.map((d, idx) => {
                  ...
                  const currency = typeof d === 'object' ? d.currency : '$';
                  return (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(59,130,246,0.15)', padding: '12px', borderRadius: '8px', border: '1px solid rgba(59,130,246,0.3)' }}>
                      <span style={{ fontSize: '13px', color: '#3b82f6', fontWeight: '500' }}>{name}</span>
                      <span style={{ fontSize: '14px', color: '#3b82f6', fontWeight: '600' }}>{price} {currency}</span>
                    </div>
                  );
                })}
              </div>
          ...
          {/* İngiltere */}
          {(appSettings?.visaDurations?.uk || []).length > 0 && (
            <div style={{ background: 'rgba(139,92,246,0.1)', padding: '14px', borderRadius: '10px', border: '1px solid rgba(139,92,246,0.2)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                <span style={{ fontSize: '18px' }}>🇬🇧</span>
                <span style={{ fontSize: '13px', fontWeight: '600', color: '#8b5cf6' }}>İngiltere</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {appSettings.visaDurations.uk.map((d, idx) => {
                  ...
                  const currency = typeof d === 'object' ? d.currency : '£';
                  return (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(139,92,246,0.15)', padding: '12px', borderRadius: '8px', border: '1px solid rgba(139,92,246,0.3)' }}>
                      <span style={{ fontSize: '13px', color: '#8b5cf6', fontWeight: '500' }}>{name}</span>
                      <span style={{ fontSize: '14px', color: '#8b5cf6', fontWeight: '600' }}>{price} {currency}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{ background: 'rgba(239,68,68,0.1)', padding: '14px', borderRadius: '10px', border: '1px solid rgba(239,68,68,0.2)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {appSettings.visaDurations.china.map((d, idx) => {
                  ...
                  const currency = typeof d === 'object' ? d.currency : '€';
                  return (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(239,68,68,0.15)', padding: '12px', borderRadius: '8px', border: '1px solid rgba(239,68,68,0.3)' }}>
                      <span style={{ fontSize: '13px', color: '#ef4444', fontWeight: '500' }}>{name}</span>
                      <span style={{ fontSize: '14px', color: '#ef4444', fontWeight: '600' }}>{price} {currency}</span>
                    </div>
                  );
                })}
              </div>
...
export default function App() {
  ...
  // Supabase'den yükle - SONRA (localStorage'ı override eder)
  ...
  const loadDataFromSupabase = async () => {
    setIsLoading(true);
    try {
      // Supabase'den yükle - sadece veri varsa güncelle
      ...
      // Hata olursa localStorage'daki veri zaten yüklenmiş olacak
    }
    setIsLoading(false);
  };
